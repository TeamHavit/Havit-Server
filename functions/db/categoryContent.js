...
const getCategoryContentByFilterAndSeen = async (client, userId, categoryId, seen, filter) => {
    ...
    const { rows } = await client.query(
        ...
        WHERE c.id = $2 AND c2.user_id = $1 AND c2.is_deleted = FALSE AND c2.is_seen = ${seen}
        ORDER BY ${filter} DESC
        `,
        [userId, categoryId]
    );
...
module.exports = { getAllCategoryContentByFilter, getCategoryContentByFilterAndSeen, addCategoryContent, deleteCategoryContentByCategoryId, deleteCategoryContentByContentId };